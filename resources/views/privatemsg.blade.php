        ...
        <div class="row">
            <div class="col-md-9">
                <div class="panel panel-primary">
                    ...
                    <input type="text" name="sendersocketid" id="sendersocketid">
                    <input type="text" name="receiversocketid" id="receiversocketid">
...
@section('js')
    ...
    <script>
        jQuery(function($) {
            ...
            var $sendersocketid = $('#sendersocketid');
            var $receiversocketid = $('#receiversocketid');
            ...
            $messageForm.on('submit', function (e) {
                ...
                socket.emit('privatesend', {sender: $sendersocketid.val(),receiver: $receiversocketid.val(),msg: $messageBox.val(), nickname: '<?= Auth::user()->name ?>'});
                $messageBox.val('');
            });
            ...
            socket.on('chat.users', function (nicknames) {
                var html = '<select id=receiver onchange="GetSelectedTextValue(this)">';

                $.each(nicknames, function (index, value) {
                    html += '<li><option  value="' + value.socketId + '" >' + value.nickname + '</option></li>';
                    if(value.nickname=='<?= Auth::user()->name ?>'){
                        $sendersocketid.val(value.socketId);
                    }
                });
    ...
    <script type="text/javascript">
        function GetSelectedTextValue(ddlFruits) {
            document.getElementById("receiversocketid").value=ddlFruits.value;
